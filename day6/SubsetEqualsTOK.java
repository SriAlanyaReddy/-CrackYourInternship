...
public class Solution {
    public static boolean subsetSumToK(int n, int k, int a[]){
        ...
        int ind=n-1;
        return sumEquls(ind, k, a);
       
    }
    public static boolean sumEquls(int ind,int target,int a[])
    {
         if(target==0){
            return true;
        }
        ...
        boolean take=false;
        boolean nottake=sumEquls(ind-1, target, a);
        if(target>=a[ind]){
            take=sumEquls(ind-1,target-a[ind],a);
        }
        return take | nottake;
    }
}
